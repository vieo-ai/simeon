...
import { IpcOrigin, IpcMessageType, IpcMessage, ipcMessageSchema, Ack, TaskCommand, TaskEvent } from "@evals/types"
...
export class IpcClient extends EventEmitter<IpcClientEvents> {
	...
	constructor(socketPath: string, log = console.log) {
		...
		this._id = `simeon-evals-${crypto.randomBytes(6).toString("hex")}`
		this._log = log
