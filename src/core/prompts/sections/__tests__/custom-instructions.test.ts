...
describe("loadRuleFiles", () => {
	...
	it("should read and trim file content", async () => {
		// Simulate no .sime/rules directory
	...
	it("should use .sime/rules/ directory when it exists and has files", async () => {
		// Simulate .sime/rules directory exists
		...
		readdirMock.mockResolvedValueOnce([
			{ name: "file1.txt", isFile: () => true, isSymbolicLink: () => false, parentPath: "/fake/path/.sime/rules" },
			{ name: "file2.txt", isFile: () => true, isSymbolicLink: () => false, parentPath: "/fake/path/.sime/rules" },
		] as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			if (filePath.toString() === "/fake/path/.sime/rules/file1.txt") {
				return Promise.resolve("content of file1")
			}
			if (filePath.toString() === "/fake/path/.sime/rules/file2.txt") {
		...
		const result = await loadRuleFiles("/fake/path")
		expect(result).toContain("# Rules from /fake/path/.sime/rules/file1.txt:")
		expect(result).toContain("content of file1")
		expect(result).toContain("# Rules from /fake/path/.sime/rules/file2.txt:")
		...
		expect(statMock).toHaveBeenCalledWith("/fake/path/.sime/rules")
		expect(statMock).toHaveBeenCalledWith("/fake/path/.sime/rules/file1.txt")
		expect(statMock).toHaveBeenCalledWith("/fake/path/.sime/rules/file2.txt")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.sime/rules/file1.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.sime/rules/file2.txt", "utf-8")
	})

	it("should fall back to .roorules when .sime/rules/ is empty", async () => {
		// Simulate .sime/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: jest.fn().mockReturnValue(true),
		} as any)
	...
	it("should read files from nested subdirectories in .sime/rules/", async () => {
		// Simulate .sime/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: jest.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			{
				...
				isDirectory: () => true,
				parentPath: "/fake/path/.sime/rules",
			},
			{
				name: "root.txt",
				isFile: () => true,
			...
			},
			{
				...
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.sime/rules/subdir",
			},
			{
				name: "nested2.txt",
				isFile: () => true,
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.sime/rules/subdir/subdir2",
			},
		] as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (path === "/fake/path/.sime/rules/root.txt") {
				return Promise.resolve("root file content")
			}
			if (path === "/fake/path/.sime/rules/subdir/nested1.txt") {
				return Promise.resolve("nested file 1 content")
			}
			if (path === "/fake/path/.sime/rules/subdir/subdir2/nested2.txt") {
		...
		expect(result).toContain("# Rules from /fake/path/.sime/rules/root.txt:")
		expect(result).toContain("root file content")

		// Check nested files content
		expect(result).toContain("# Rules from /fake/path/.sime/rules/subdir/nested1.txt:")
		expect(result).toContain("nested file 1 content")
		expect(result).toContain("# Rules from /fake/path/.sime/rules/subdir/subdir2/nested2.txt:")
		expect(result).toContain("nested file 2 content")

		// Verify correct paths were checked
		expect(statMock).toHaveBeenCalledWith("/fake/path/.sime/rules/root.txt")
		expect(statMock).toHaveBeenCalledWith("/fake/path/.sime/rules/subdir/nested1.txt")
		expect(statMock).toHaveBeenCalledWith("/fake/path/.sime/rules/subdir/subdir2/nested2.txt")

		// Verify files were read with correct paths
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.sime/rules/root.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.sime/rules/subdir/nested1.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.sime/rules/subdir/subdir2/nested2.txt", "utf-8")
	})
})
...
describe("addCustomInstructions", () => {
	...
	it("should combine all instruction types when provided", async () => {
		// Simulate no .sime/rules-test-mode directory
		statMock.mockRejectedValueOnce({ code: "ENOENT" })
	...
	it("should use .sime/rules-test-mode/ directory when it exists and has files", async () => {
		// Simulate .sime/rules-test-mode directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: jest.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			{
				...
				isFile: () => true,
				isSymbolicLink: () => false,
				parentPath: "/fake/path/.sime/rules-test-mode",
			},
			{
				name: "rule2.txt",
				isFile: () => true,
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			if (filePath.toString() === "/fake/path/.sime/rules-test-mode/rule1.txt") {
				return Promise.resolve("mode specific rule 1")
			}
			if (filePath.toString() === "/fake/path/.sime/rules-test-mode/rule2.txt") {
		...
		expect(result).toContain("# Rules from /fake/path/.sime/rules-test-mode")
		expect(result).toContain("# Rules from /fake/path/.sime/rules-test-mode/rule1.txt:")
		expect(result).toContain("mode specific rule 1")
		expect(result).toContain("# Rules from /fake/path/.sime/rules-test-mode/rule2.txt:")
		expect(result).toContain("mode specific rule 2")

		expect(statMock).toHaveBeenCalledWith("/fake/path/.sime/rules-test-mode")
		expect(statMock).toHaveBeenCalledWith("/fake/path/.sime/rules-test-mode/rule1.txt")
		expect(statMock).toHaveBeenCalledWith("/fake/path/.sime/rules-test-mode/rule2.txt")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.sime/rules-test-mode/rule1.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.sime/rules-test-mode/rule2.txt", "utf-8")
	})

	it("should fall back to .roorules-test-mode when .sime/rules-test-mode/ does not exist", async () => {
		// Simulate .sime/rules-test-mode directory does not exist
		statMock.mockRejectedValueOnce({ code: "ENOENT" })
	...
	it("should fall back to .clinerules-test-mode when .sime/rules-test-mode/ and .roorules-test-mode do not exist", async () => {
		// Simulate .sime/rules-test-mode directory does not exist
		statMock.mockRejectedValueOnce({ code: "ENOENT" })
	...
	it("should correctly format content from directories when using .sime/rules-test-mode/", async () => {
		...
		readdirMock.mockResolvedValueOnce([
			{ name: "rule1.txt", isFile: () => true, parentPath: "/fake/path/.sime/rules-test-mode" },
		] as any)
		readFileMock.mockReset()
		...
		statMock.mockImplementation((filePath) => {
			statCallCount++
			if (filePath === "/fake/path/.sime/rules-test-mode/rule1.txt") {
				return Promise.resolve({
					isFile: jest.fn().mockReturnValue(true),
					isDirectory: jest.fn().mockReturnValue(false),
				} as any)
...
describe("Rules directory reading", () => {
	it("should follow symbolic links in the rules directory", async () => {
		...
		readdirMock
			...
			.mockResolvedValueOnce([
				{ name: "subdir_link.txt", isFile: () => true, parentPath: "/fake/path/.sime/rules/symlink-target-dir" },
			] as any)
		...
		statMock.mockImplementation((path: string) => {
			// For directory check
			if (path === "/fake/path/.sime/rules" || path.endsWith("dir")) {
				return Promise.resolve({
					isDirectory: jest.fn().mockReturnValue(true),
					isFile: jest.fn().mockReturnValue(false),
				} as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			if (filePath.toString() === "/fake/path/.sime/rules/regular.txt") {
				return Promise.resolve("regular file content")
			}
			if (filePath.toString() === "/fake/path/.sime/rules/../symlink-target.txt") {
				return Promise.resolve("symlink target content")
			}
			if (filePath.toString() === "/fake/path/.sime/rules/symlink-target-dir/subdir_link.txt") {
				return Promise.resolve("regular file content under symlink target dir")
			}
			if (filePath.toString() === "/fake/path/.sime/rules/../nested-symlink-target.txt") {
		...
		expect(result).toContain("# Rules from /fake/path/.sime/rules/regular.txt:")
		expect(result).toContain("regular file content")
		expect(result).toContain("# Rules from /fake/path/.sime/rules/../symlink-target.txt:")
		expect(result).toContain("symlink target content")
		expect(result).toContain("# Rules from /fake/path/.sime/rules/symlink-target-dir/subdir_link.txt:")
		expect(result).toContain("regular file content under symlink target dir")
		expect(result).toContain("# Rules from /fake/path/.sime/rules/../nested-symlink-target.txt:")
		expect(result).toContain("nested symlink target content")

		// Verify readlink was called with the symlink path
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.sime/rules/link.txt")
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.sime/rules/link_dir")

		// Verify both files were read
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.sime/rules/regular.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.sime/rules/../symlink-target.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.sime/rules/symlink-target-dir/subdir_link.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.sime/rules/../nested-symlink-target.txt", "utf-8")
	})
	beforeEach(() => {
		jest.clearAllMocks()
	})
	...
	it("should correctly format multiple files from directory", async () => {
		...
		// Simulate listing files
		readdirMock.mockResolvedValueOnce([
			{ name: "file1.txt", isFile: () => true, parentPath: "/fake/path/.sime/rules" },
			{ name: "file2.txt", isFile: () => true, parentPath: "/fake/path/.sime/rules" },
			{ name: "file3.txt", isFile: () => true, parentPath: "/fake/path/.sime/rules" },
		] as any)

		statMock.mockImplementation((path) => {
			expect([
				"/fake/path/.sime/rules/file1.txt",
				"/fake/path/.sime/rules/file2.txt",
				"/fake/path/.sime/rules/file3.txt",
			]).toContain(path)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (filePath.toString() === "/fake/path/.sime/rules/file3.txt") {
		...
		expect(result).toContain("# Rules from /fake/path/.sime/rules/file2.txt:")
		expect(result).toContain("content of file2")
		expect(result).toContain("# Rules from /fake/path/.sime/rules/file3.txt:")
