...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	return path.join(cwd, ".sime", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .sime/system-prompt-[mode slug]
...
/**
 * Ensures the .sime directory exists, creating it if necessary
 */
export async function ensureRooDirectory(cwd: string): Promise<void> {
	const rooDir = path.join(cwd, ".sime")
